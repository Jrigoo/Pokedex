...
import { IPokemon, IPokemons } from "../utils/interfaces";
...
export async function getPokemons(
  ...
  try {
    ...
    return { nextURL: data.next, pokedex: pokedex };
  } catch (error) {
    console.error(error);
  }
}
...
export async function getFavPokemons(ids: Array<number>) {
  try {
    const favorites = [];
